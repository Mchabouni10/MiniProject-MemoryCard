...
const shuffleAndCreateGrid = (MyCards) => {
  ...
  for (let i = 0; i < MyCards.length; i++) {
    ...
    card.addEventListener("click", () => {
        card.classList.toggle("turnOver");
...
function startGameTimer() {
  ...
  timerInterval = setInterval(() => {
    //setInterval is built Javascript function that count
...
function reveal2chosenCard(card) {
  ...
  if (MyClickedCards.length === 2) {
    //if there is just 2 cards selected
    waitForMove = 2; // waitForMove is set 2 to for preventing to compare more than 2 cards
    const firstCard = MyClickedCards[0];
    const secondCard = MyClickedCards[1];

    // Compare the two selected cards
    if (firstCard.dataset.MyCards === secondCard.dataset.MyCards) {
      // statement to check or compare if both cards are matched
      // Match found

      setTimeout(() => {
        //give the affect to show that two card as matched and have the same image
        ...
        if (twoMatchedCards === cardNumbers) {
          //check if the player one won means our twoMatchedCard is 16
      ...
      //else statament if the 2 cards doesn

      setTimeout(() => {
        //use function setTimeout to two turnedOver cards
        MyClickedCards.forEach((card) => {
          //iterate my Array
